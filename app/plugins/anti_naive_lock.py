"""
Anti-Naive-Lock Preprocessing Plugin
...
class AntiNaiveLockPlugin:
    """
    Plugin for applying selective preprocessing to prevent naive lock.
    ...
    def process_sliding_windows(self, 
        ...
        if not config.get('anti_naive_lock_enabled', False):
            self.logger.info("Anti-naive-lock preprocessing disabled")
            return x_train, x_val, x_test, {}
            
        self.logger.info("Starting anti-naive-lock preprocessing...")
    ...
    def _apply_selective_preprocessing(self, 
        ...
        price_features = config.get('price_features', [])
        ...
        constant_daily_features = config.get('constant_daily_features', [])
        ...
        for i, feature_name in enumerate(feature_names):
            try:
                if feature_name in price_features and config.get('use_log_returns', True):
                    ...
                    self.logger.debug(f"Applied log returns to {feature_name}")
                    
                elif feature_name in temporal_features and config.get('use_cyclic_encoding', True):
                    # Apply cyclic encoding to temporal features
                    sin_vals, cos_vals = self._apply_cyclic_encoding(x_train_processed[:, :, i], feature_name)
                    # Note: This creates 2 features from 1, need to handle in calling code
                    processing_stats['applied_transforms'][feature_name] = 'cyclic_encoding'
                    self.logger.debug(f"Applied cyclic encoding to {feature_name}")
                    ...
                    self.logger.debug(f"Applied first differences to {feature_name}")
                    ...
                    x_test_processed[:, :, i] = self._apply_log_returns(x_test_processed[:, :, i])
                    processing_stats['applied_transforms'][feature_name] = 'log_returns'
                    self.logger.debug(f"Applied log returns to sub-periodicity {feature_name}")
                    ...
                    self.logger.debug(f"Applied daily differences to {feature_name}")
        ...
        self.logger.info(f"Anti-naive-lock preprocessing completed. Transforms applied to {len([k for k, v in processing_stats['applied_transforms'].items() if v != 'preserved'])} features")
    ...
    def _apply_cyclic_encoding(self, feature_data: np.ndarray, feature_name: str) -> Tuple[np.ndarray, np.ndarray]:
        """
        Apply cyclic encoding: sin(2π * x / period), cos(2π * x / period)
    ...
    def _apply_uniform_log_returns(self, 
        ...
        for i, feature_name in enumerate(feature_names):
            try:
                ...
                self.logger.error(f"Error applying uniform log returns to {feature_name}: {e}")
                processing_stats['applied_transforms'][feature_name] = f'error: {str(e)}'
        
        self.logger.info("Applied uniform log returns to all features")
        return x_train_processed, x_val_processed, x_test_processed, processing_stats
    ...
    def _apply_feature_normalization(self, 
        ...
        for i in range(x_train.shape[2]):
            ...
            if np.any(valid_mask):
                ...
                self.logger.warning(f"Feature {feature_names[i]} has no valid values for normalization")
