...
class Plugin:
    ...
    def process(self, data, method='granger', save_params=None, load_params=None, max_lag=5, significance_level=0.05, column=None, columns=None):
        ...
        Args:
            ...
            column (int): The index of the column to select (used for 'select_single').
            columns (list of int): The indices of the columns to select (used for 'select_multi').
        ...
        if load_params and os.path.exists(load_params):
            ...
            if method == 'acf':
                ...
                if column is None:
                    raise ValueError("Column index must be specified for select_single method")
                selected_features = [data.columns[column]]
            elif method == 'select_multi':
                if columns is None:
                    raise ValueError("Column indices must be specified for select_multi method")
