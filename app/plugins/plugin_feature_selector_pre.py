...
class Plugin:
    ...
    def process(self, data, method='granger', save_params=None, load_params=None, max_lag=5, significance_level=0.05, column=None, columns=None):
        ...
        Args:
            ...
            column (int): Column index for 'select_single' method.
            columns (list): List of column indices for 'select_multi' method.
        ...
        if self.feature_selection_params is None:
            if method == 'acf':
                ...
                selected_features = [data.columns[i] for i in columns]
            else:
    ...
    def _granger_causality_feature_selection(self, data, max_lag, significance_level):
        selected_features = []
        target_column = data.columns[0]  # Assuming the first column is the target
        for column in data.columns:
