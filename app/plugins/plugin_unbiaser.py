...
class Plugin:
    ...
    plugin_debug_vars = ['method', '_size', 'ema_alpha']
    ...
    def get_debug_info(self):
        ...
        Returns:
            dict: Debug information including method, _size, and ema_alpha.
        """
    ...
    def process(self, data):
        ...
        _size = int(self.params.get('_size', 5))  # Ensure _size is an integer
        ema_alpha = float(self.params.get('ema_alpha', 0.1))  # Ensure ema_alpha is a float

        print("Starting the process method.")
        print(f"Method: {method},  size: {_size}, EMA alpha: {ema_alpha}")
        ...
        if method == 'ma':
            ...
            processed_data = self._moving_average_unbias(data[numeric_columns], _size)
        elif method == 'ema':
    ...
    def _moving_average_unbias(self, data, _size):
        """
        Apply moving average unbiasing to the data.

        Args:
            data (pd.DataFrame): The input data to be processed.
            _size (int): The  size for the moving average.

        Returns:
            pd.DataFrame: The unbiassed data.
        """
        print(f"Applying moving average with  size: {_size}")
        unbiassed_data = data.astype(float).copy()  # Ensure all data is float

        for col in data.columns:
            print(f"Processing column: {col}")
            for i in range(len(data)):
                 = data[col][max(0, i-_size+1):i+1].mean()
                unbiassed_data.at[data.index[i], col] = data.at[data.index[i], col] - 
